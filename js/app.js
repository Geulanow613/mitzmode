...
class MitzMode {
    ...
    async playRewardVideo(videoNumber) {
        ...
        return new Promise((resolve, reject) => {
            const isFinalReward = videoNumber === CONFIG.REWARDS.FINAL_REWARD_VIDEO_ID;
            const dialog = document.createElement('div');
            dialog.className = 'dialog video-dialog' + (isFinalReward ? ' final-reward' : '');
            const videoPath = isFinalReward
                ? CONFIG.REWARDS.FINAL_REWARD_ASSET
                : `${CONFIG.REWARDS.MILESTONE_ASSET_PREFIX}${videoNumber}.mp4`;
            console.log('Attempting to play video from path:', videoPath);

            // Final reward has no rainbow "New Level Achieved" overlay — video only.
            const overlayHTML = isFinalReward
                ? ''
                : (() => {
                    const info = this.getLevelInfo(videoNumber);
                    const starCount = Math.max(0, Math.min(13, videoNumber));
                    const stars = Array(starCount).fill('✡')
                        .map((s, i) => `<span style="animation-delay: ${i * 0.1}s">${s}</span>`)
                        .join('');
                    return `
                    <div class="level-up-overlay rainbow">
                        <div class="level-up-card">
                            <div class="star-burst"></div>
                            <h2>New Level Achieved!</h2>
            ...
            dialog.innerHTML = `
                <div class="dialog-content">
                    <video id="rewardVideo" playsinline autoplay></video>
                    ${overlayHTML}
                    <div class="reward-controls">
                        <button class="reward-control mute-button" aria-label="Mute" title="Mute">
                            <span class="icon-unmute">🔊</span>
                            <span class="icon-mute" style="display:none">🔇</span>
                        </button>
                        <button class="reward-control close-reward-button" aria-label="Close" title="Close">×</button>
                    </div>
                </div>
            `;
            ...
            // Final reward starts unmuted (audio is intentional); milestone videos also unmuted by default.
            video.muted = false;
            video.volume = 1.0;
            ...
            video.onerror = (e) => {
                ...
                // Some browsers block unmuted autoplay — try muted as a fallback before giving up.
                if (!video.muted) {
                    video.muted = true;
                    video.play().catch(() => reject(new Error('Failed to load video')));
                } else {
            ...
            video.onended = closeAndResolve;
            dialog.querySelector('.close-reward-button').onclick = closeAndResolve;

            const muteBtn = dialog.querySelector('.mute-button');
            ...
            // Try to start playback — most browsers require user gesture for unmuted; we already had one.
            const playPromise = video.play();
            if (playPromise && playPromise.catch) {
                playPromise.catch(() => {
                    // Fall back to muted autoplay so user still sees the clip.
                    video.muted = true;
                    iconUnmute.style.display = 'none';
                    iconMute.style.display = '';
                    muteBtn.setAttribute('aria-label', 'Unmute');
    ...
    checkTzaddikStatus() {
        ...
        // Milestone tiers (parity with Android viewmodel). Index = milestone number 1..14.
        // The 14th tier ("Mitz Mode!") uses the FINAL_REWARD_VIDEO_ID sentinel rather than a
        // numeric mitzmodenew{N}.mp4 — see playRewardVideo().
        const levels = [
        ...
        if (currentLevel > lastAchievedLevel) {
            ...
            if (currentLevel > 0 && currentLevel <= levels.length) {
                ...
                const isMitzMode = levelInfo.name === "Mitz Mode!";
                const videoId = isMitzMode ? CONFIG.REWARDS.FINAL_REWARD_VIDEO_ID : currentLevel;

                // Final reward tier: video only (no rainbow level-up dialog before it).
                if (!isMitzMode) {
                ...
                console.log('Attempting to play reward video for level:', currentLevel, 'videoId:', videoId);
                this.playRewardVideo(videoId).then(() => {
                    console.log('Video playback completed successfully');
                    this.showNotification(`Mazel Tov! You've reached: ${levelInfo.name}!`, 'success');
                }).catch(error => {
    ...
    /** Replay the 1800+ Mitz Mode! reward video from the certificate. */
    replayFinalRewardVideo() {
        this.playRewardVideo(CONFIG.REWARDS.FINAL_REWARD_VIDEO_ID).catch(error => {
            console.error('Error replaying final reward video:', error);
        });
    }
    ...
    getLevelInfo(levelNumber) {
        if (levelNumber === CONFIG.REWARDS.FINAL_REWARD_VIDEO_ID) {
            return { name: "Mitz Mode!" };
        }
        const levels = [
            { name: "Beginner" },
            { name: "Ba'al Teshuva" },
    ...
    getNextLevel(currentCount) {
        const levels = [
            ...
            { threshold: 1800, title: "Mitz Mode!" }
        ];
    ...
    showMitzvahDialog(mitzvah) {
        ...
        acceptButton.onclick = () => {
            ...
            DialogManager.playHolyLightFlash();
